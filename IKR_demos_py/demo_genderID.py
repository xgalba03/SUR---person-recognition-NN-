...
from ikrlib import raw8khz2mfcc, logpdf_gauss, train_gauss, train_gmm, logpdf_gmm, wav16khz2mfcc
import scipy.linalg
import numpy as np
...
train_m = raw8khz2mfcc('gID_data/male/train').values()
train_f = raw8khz2mfcc('gID_data/female/train').values()
test_m = raw8khz2mfcc('gID_data/male/test').values()
test_f = raw8khz2mfcc('gID_data/female/test').values()

# train_m = wav16khz2mfcc('train_data/non_target_train').values()
# train_f = wav16khz2mfcc('train_data/target_train').values()
# test_m = wav16khz2mfcc('train_data/non_target_dev').values()
# test_f = wav16khz2mfcc('train_data/target_dev').values()
...
plt.figure()
junk = plt.hist(train_m.dot(e), 40, histtype='step', color='b', density=True)
junk = plt.hist(train_f.dot(e), 40, histtype='step', color='r', density=True)
plt.show()
...
# m_list = list(test_m[0].values())
m_anchor = list(test_m)[0]
train_m_list = list(train_m)
train_f_list = list(train_f)

ll_m = logpdf_gauss(m_anchor, np.mean(train_m_list, axis=0), np.var(train_m_list, axis=0))
ll_f = logpdf_gauss(m_anchor, np.mean(train_f_list, axis=0), np.var(train_f_list, axis=0))
...
print(float(sum(ll_m) + np.log(P_m)) - (sum(ll_f) + np.log(P_f)))

# Repeating the whole excercise, now with gaussian models with full covariance
# matrices

ll_m = logpdf_gauss(list(test_m)[0], *train_gauss(train_m))
ll_f = logpdf_gauss(list(test_m)[0], *train_gauss(train_f))
...
ll_m = logpdf_gauss(list(test_f)[1], *train_gauss(train_m))
ll_f = logpdf_gauss(list(test_f)[1], *train_gauss(train_f))
# '*' before 'train_gauss' pases both return values (mean and cov) as parameters of 'logpdf_gauss' 
posterior_m = np.exp(ll_m) * P_m / (np.exp(ll_m) * P_m + np.exp(ll_f) * P_f);
plt.figure();
plt.plot(posterior_m, 'b');
...
for jj in range(30):
    ...
    print('Iteration:', jj, ' Total log-likelihood:', TTL_m, 'for males;', TTL_f, 'for females')
